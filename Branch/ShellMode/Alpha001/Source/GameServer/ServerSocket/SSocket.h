...
protected: // interface
	...
	virtual int			SendPacket2Net();//07.5.21彭正帮添加
	virtual int			GetSendLeaveLen();

	bool				GetPeerStrIP( char* szClientIP );//  [6/1/2007]彭正帮添加

	virtual int			GetRecvBufferLen()	{ return m_nLen;	}				
	virtual const char* GetRecvBuffer()		{ return m_bufMsg;	}
	
	virtual bool		CopyDataToBuffer( const char* pSource,int nLen );	//  [7/11/2007]彭正帮添加						


	int	ProcessSendFail( int nRet )//  [7/11/2007] 彭正帮添加失败处理
	{		
		if( 0 == nRet )
		{
			LOGERROR("ERROR: SendPacket2Net send ret 0, Client Closed!");
			Close( true );
		}
		else
		{
			...
			if( nErrCode != WSAEWOULDBLOCK )
			{
				...
				{
					LOGERROR("ERROR: SendPacket2Net, Network Error, Socket Close, ErrCode:%d",nErrCode );
				}
				Close( true );
			}
		}
