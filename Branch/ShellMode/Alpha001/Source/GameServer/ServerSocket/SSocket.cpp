...
// #endif
		...
		else if(ret < 0)
		{
			...
			if(err != WSAEWOULDBLOCK)
			{
				...
				{
					LOGERROR( "ERROR,GetPacket(), Network Error! ErrCode[%d]",err );
				}
...
void CServerSocket::Close(BOOL bNoLinger /*= false*/)
{
	if(m_sock != INVALID_SOCKET)
	{
		LOGDEBUG("SOCKET Closeing !");
		closesocket(m_sock);
		m_sock = INVALID_SOCKET;
		LOGDEBUG("SOCKET Closeed !");
		//? bNoLinger暂未使用
	}
//	m_nLen = 0;
}
...
bool CServerSocket::CopyDataToBuffer( const char* pSource,int nLen )
{
	...
	{
		LOGWARNING( "CopyDataToBuffer Error,socket Close!" );
		Close( true );
		return false;
	}
