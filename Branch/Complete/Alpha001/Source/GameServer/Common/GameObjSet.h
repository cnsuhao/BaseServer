...
public:
	...
	virtual void		EraseObj(T* pObj)		PURE_VIRTUAL_FUNCTION
...
// }
template<class T>
void CGameObjSet<T>::Clear(bool bDel)
{
	T* pObj=NULL;
	for(VECTOR_SET::iterator it=m_setObj.begin();it!=m_setObj.end();)
	{
		pObj=*it;
		it=m_setObj.erase(it);
		if(bDel && m_bOwner && pObj)
			pObj->ReleaseByOwner();
	}
}
...
bool CGameObjSet<T>::AddObj(T* pObj)
{
	...
	UINT Objectid=pObj->GetID();
	if(DelObj(pObj->GetID()))
	{
		tolog3("CGameObjSet<T>::AddObj DelObj(pObj->GetID()) check!!id:%d,name:%s",Objectid,typeid(pObj).name());
	}
...
//////////////////////////////////////////////////////////////////////
template<class T>
void CGameObjSet<T>::EraseObj(T* pObj)
{
	CHECK(pObj);
	T* pInObj=NULL;
	for(int i=0;i<m_setObj.size();i++)
