...
bool CMessagePort::PushMsg(int nPort, int nPacket, VAR_TYPE nVarType, const void* buf)	// nData中的串和结构都会被COPY
{
	...
	IF_NOT (pMsg)
	{
		int nNowSize = GetMsgSize(); 
		int nRecycleSize = m_setRecycle.size();
		::LogSave("Error! CMessagePacket::PushMsg new CMessagePackket Failed! NowSize[%d], nRecycleSize[%d]", nNowSize, nRecycleSize);
	...
	if (nSetSize >= _warningMessagePacket && (0 == nSetSize % 1000))
	{
		::LogSave("Warning! CMessagePacket::PushMsg[%d] SetSize[%u]。[%d][%d][%s]", m_id, nSetSize, nPort, nPacket, (LPCTSTR)DumpBuffer((char*)buf, __min(16,nSize)));
		InterlockedExchange(&g_nMessagePortErrorCount, nSetSize);
	}

	// 容器超大关闭
// 	const int _maxMessagePacket=400000;//20070302.原10W...测试
...
{
	MSG_SET::iterator iter = m_setMsg.begin();
	for(;iter != m_setMsg.end(); iter++)
	{
		SAFE_DELETE(*iter);
	}
	m_setMsg.clear();
