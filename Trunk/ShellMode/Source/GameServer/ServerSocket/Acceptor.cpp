...
{
	CHECK( dwNumberOfThreads <= MAX_ACCEPT_THREAD );
...
BOOL Acceptor::CreateListenSocket( char *pIP, WORD wPort )
{
	...
	::SetClientSockOpt(m_listenSocket);
...
VOID Acceptor::SuspendListenThread()
{
	for( DWORD i = 0; i < m_numThreads; ++i )
	{
		CHECKC( m_hThread[i] != INVALID_HANDLE_VALUE );
...
VOID Acceptor::ResumeListenThread()
{
	for( DWORD i = 0; i < m_numThreads; ++i )
	{
		CHECKC( m_hThread != INVALID_HANDLE_VALUE );
